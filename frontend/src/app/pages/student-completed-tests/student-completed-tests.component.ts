...
@Component({
  selector: 'app-student-completed-tests',
  standalone: true,
    ...
    <div class="student-tests">
      <header class="student-tests__header">
        ...
        <div class="student-tests__title-box">
          <h1 class="student-tests__title">Пройденные тесты</h1>
        </div>
      ...
      <section class="student-tests__board">
        ...
        <ul *ngIf="!loading && tests.length > 0" class="student-tests__list">
          <li *ngFor="let t of tests" class="student-tests__item">
            <a [routerLink]="['/student/tests/completed', t.id]" class="student-tests__open">
              <h2 class="student-tests__name">{{ t.name }}</h2>
              <div class="student-tests__completed" *ngIf="t.completedAt">
                <strong>Завершён:</strong> {{ t.completedAt | date : 'dd.MM.yyyy, HH:mm' }}
              </div>
              <div class="student-tests__dates">
                <span class="student-tests__date"
                  ><strong>Период теста:</strong> {{ t.startDate | date : 'dd.MM.yyyy, HH:mm' }} —
                  {{ t.endDate | date : 'dd.MM.yyyy, HH:mm' }}</span
                >
              </div>
              ...
              <p class="student-tests__hint" *ngIf="t.totalTimeSeconds != null">
                Время прохождения: {{ formatDuration(t.totalTimeSeconds) }}
              </p>
            </a>
          </li>
        </ul>
  ...
  styles: [
    `
      ...
      .student-tests__open:hover {
        background: #f1f5f9;
      }

      .student-tests__open:focus-visible {
...
/**
 * Компонент списка завершенных тестов студента.
 */
export class StudentCompletedTestsComponent implements OnInit {
  ...
  /**
   * Загружает завершенные тесты студента.
   */
  ngOnInit(): void {
    this.auth.getStudentCompletedTests().subscribe({
      next: (list) => {
        this.tests = list ?? [];
        this.loading = false;
      },
