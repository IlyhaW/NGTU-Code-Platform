...
@Component({
  selector: 'app-student-current-tests',
  standalone: true,
    ...
    <div class="student-tests">
      <header class="student-tests__header">
        ...
        <div class="student-tests__title-box">
          <h1 class="student-tests__title">Текущие тесты</h1>
        </div>
...
/**
 * Компонент списка текущих тестов студента.
 */
export class StudentCurrentTestsComponent implements OnInit {
  ...
  /**
   * Загружает текущие тесты студента.
   */
  ngOnInit(): void {
    this.auth.getStudentCurrentTests().subscribe({
      next: (list) => {
        this.tests = list ?? [];
        this.loading = false;
      },
      error: (err: HttpErrorResponse) => {
        ...
        else this.error = 'Не удалось загрузить список тестов.';
      },
    });
  }
}
